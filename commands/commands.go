...
type Command struct {
	...
	CommandResponse func(s *discordgo.Session, m *discordgo.MessageCreate, arg1 string)
	Admin           bool
	Args            int
	Desc            string
}
...
func RegisterCommands() {
	...
	//SoftMod Commands
	CL.CommandList = append(CL.CommandList, Command{Name: "players", Command: utils.PlayersList, CommandResponse: utils.PlayersListResponse,
		Admin: false, Args: 1, Desc: "Unban a user from the server. Usage !unban <player>"})
...
// RunCommandResponse runs a specified command response that was generated by the softmod.
func RunCommandResponse(input string, s *discordgo.Session, m *discordgo.MessageCreate) {
	inputvars := strings.SplitN(input, " ", 3)
	for _, command := range CL.CommandList {
		if strings.ToLower(command.Name) == strings.ToLower(inputvars[0]) && command.CommandResponse != nil {
			//s.ChannelMessageSend(m.ChannelID, "0 arguments!")
			command.CommandResponse(s, m, inputvars[1])
			return
		}
	}
	s.ChannelMessageSend(m.ChannelID, "CommandResponse not found or the CommandResponse is missing required arguments!")
}
